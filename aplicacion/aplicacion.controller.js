    ...
    if(err){
      ...
      dbs.query(newsql,function(error,data){
        ...
        let sqlTrazabilidad = "insert into usoAPI (ip,ll,ciudad,region,pais,funcion,request,api_key) ";
          ...
          let maquinaSql  = "select  ip_servidor , puerto , nombre_usuario, clave_usuario from maquinas_server where id_servidor = '"+ip_servidor+"' ";
          console.log(maquinaSql);
		  dbs.query(maquinaSql , function(err,res){
            ...
            let puerto = datos.puerto;
            let nombre_usuario = datos.nombre_usuario;
            let clave_usuario = datos.clave_usuario;
            ...
            if(err){
              ...
              let codCampo = req.body.codCampo || false;
              ...
              if(codCampo == parseInt(codCampo) && codCampo > 0 ){
                sql += " AND predio_trabajos_agricolas.id_predio = "+codCampo;
              }else{
                if(codCampo){
                  result.json({"Error":"#96 El valor de codCampo no cumple los requisitos"});
                  return false;
                }
              }
              let flag = 0;
